-- Code taken from:
-- https://github.com/OHDSI/ETL-CMS/blob/master/SQL/create_vocabv5_drug_era_non_stockpile.sql
...
ctePreDrugTarget(
    ...
    INNER JOIN {{ source('cdm', 'concept_ancestor') }} AS ca ON ca.descendant_concept_id = d.drug_concept_id
    INNER JOIN {{ source('cdm', 'concept') }} AS c ON ca.ancestor_concept_id = c.concept_id
...
cteFinalTarget(
    ...
    SELECT
        ...
        DATE_DIFF('day', drug_sub_exposure_end_date, drug_sub_exposure_start_date) AS days_exposed
...
SELECT
    ...
    DATE_DIFF('day', drug_era_end_date, MIN(drug_sub_exposure_start_date)) - SUM(days_exposed) AS gap_days
FROM cteDrugEraEnds
